...
class Test_init:
  ...
  def test_init(self):
    ...
    if self.driver.execute_script("return ($('[name=\"sysvar-enable\"]:checked').length == 0)"):
      None if (element := self.driver.find_element(By.NAME, "sysvar-enable")).is_selected() else element.click()
    ...
    self.driver.find_element(By.CSS_SELECTOR, "tr[data-module=\"extra_calculation_functions\"] .external-modules-configure-button").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.NAME, "sysvar-enable")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[type=\"checkbox\"][name=\"sysvar-enable\"]:checked")) > 0
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[type=\"text\"][name=\"sysvar-name____0\"]:not([value=\"\"])")) > 0
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[type=\"text\"][name=\"sysvar-value____0\"]:not([value=\"\"])")) > 0
    ...
    self.driver.find_element(By.ID, "purpose").find_element(By.XPATH, "//option[. = 'Practice / Just for fun']").click()
    ...
    time.sleep(5)
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "#external-modules-enabled tr[data-module=\"extra_calculation_functions\"]")) > 0
    ...
    self.driver.find_element(By.NAME, "demographics_complete").find_element(By.CSS_SELECTOR, "*[value='0']").click()
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"DataEntry/index.php\"]:not([href*=\"id=&\"]):not([href*=\"&auto=1\"])")) > 0
