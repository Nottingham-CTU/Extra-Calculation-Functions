...
# Test name: t08 - checkvalueoncurrentinstance fn
import pytest
import time
import json
...
class Test_08_checkvalueoncurrentinstance_fn:
  ...
  def test_08_checkvalueoncurrentinstance_fn(self):
    ...
    None if (element := self.driver.find_element(By.CSS_SELECTOR, ".repeat_form_chkbox")).is_selected() else element.click()
    self.driver.find_element(By.XPATH, "//button[contains(.,\"Save\")]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "#enableRepeatingFormsEventsOption[style*=\"color:green\"]")) > 0
    ...
    self.driver.find_element(By.CSS_SELECTOR, "[onchange*=\"checkRepeatSelection\"]").find_element(By.XPATH, "//option[. = 'Basic Demography Form']").click()
    ...
    assert len(self.driver.find_elements(By.NAME, "first_name")) > 0
    self.driver.find_element(By.LINK_TEXT, "Record Status Dashboard").click()
    ...
    self.driver.find_element(By.ID, "enableRepeatingFormsEventsBtn").click()
    self.driver.execute_script("$('#south').remove()")
    None if not (element := self.driver.find_element(By.CSS_SELECTOR, ".repeat_form_chkbox")).is_selected() else element.click()
    self.driver.find_element(By.XPATH, "//button[contains(.,\"Save\")]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
